...
import { setDepositeSlider } from "@/store/slices/modelSlice";
import { AnimatePresence, motion } from "motion/react";
import { IoCloseCircleSharp } from "react-icons/io5";
import { FaCircleDollarToSlot } from "react-icons/fa6";

const DepositeSlider: React.FC = () => {
  const isDepositeSliderVisible = useSelector(
    (state: RootState) => state.model.showDepositeSlider
  );
  const dispatch = useDispatch<AppDispatch>();

  function handleClose() {
    dispatch(setDepositeSlider({ showDepositeSlider: false }));
  }

  return (
    <AnimatePresence mode="wait">
      {isDepositeSliderVisible && (
        <motion.div
          key="sidebar-wrapper"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        >
          ...
          >
            ...
            <div className="flex items-center mt-5 gap-3 md:gap-5 border border-gray-300 p-3 rounded-lg hover:bg-gray-300 transition ease-in-out duration-300 cursor-pointer">
              ...
              <div>
                <div className="font-semibold">Deposite USDT/USDC</div>
                <div className="text-sm text-gray-700">
                  Deposite USDT/USDC to your in app wallet
                </div>
              </div>
            </div>
            <button
